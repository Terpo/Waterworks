...
public class WaterworksPacketHandler {
	...
	public static final SimpleChannel INSTANCE = NetworkRegistry.newSimpleChannel(new ResourceLocation(Reference.MODID, Reference.NETWORK),
	...
	public static void registerMessages() {
		INSTANCE.registerMessage(packetId++, TankPacket.class, TankPacket::encode, TankPacket::decode, TankPacket::consume);
		INSTANCE.registerMessage(packetId++, PumpPacket.class, PumpPacket::encode, PumpPacket::decode, PumpPacket::consume);
	...
	public static void sendToAllAround(BasePacket packet, TileEntity tE, int range) {
		...
		if (tE.getWorld().isAreaLoaded(blockPos, 0)) {
			INSTANCE.send(PacketDistributor.NEAR.with(
					() -> new TargetPoint(blockPos.getX(), blockPos.getY(), blockPos.getZ(), range, tE.getWorld().dimension.getType())),
					packet);
		}
	}
